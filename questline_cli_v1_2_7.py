"""QuestLine CLI v1.2.7

Standalone interactive CLI wrapper for the existing `questline.py` solver.

v1.2.7 is the v1.2 closing polish patch:
- Keeps the strict 2/3/6 digit input grammar.
- Keeps v1.2.6 consistency checks, duplicate-guess checks, logical-solved state,
  and post-logic probing.
- Fixes English singular/plural solved text: "1 round" vs "N rounds".
- Replay JSON now records `ui_language`, `post_logic_probe_count`, and
  `verification_delay_rounds`.
- `extra_moves_after_logical_solve` is removed in favor of clearer fields.
"""
...
def save_replay(history: History, replay: ReplayRows, solver: Any, jackpot: bool, lang: str) -> Path:
    ...
    for i in range(1, len(history)+1):
        ans = unique_answer(solver, history[:i])
        if ans:
            logical_answer = ans
            logical_round = i
            break
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    parser = argparse.ArgumentParser(description="QuestLine interactive CLI v1.2.7")
