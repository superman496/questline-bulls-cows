"""QuestLine CLI v1.2.6

Standalone interactive CLI wrapper for the existing `questline.py` solver.

v1.2.6 is a state-consistency patch over v1.2.5:
- Keeps the strict 2/3/6 digit input grammar.
- Every input, including 4b0c, must pass history consistency before it is accepted.
- Duplicate guesses are rejected unless the user uses undo / recovery to fix a mistake.
- When only one possible answer remains, QuestLine marks the game as logically solved,
  but still allows the player to keep probing as long as all feedback remains consistent.
- Replay JSON records logical_answer, logical_solved_at_round, verified_at_round,
  extra_moves_after_logical_solve, post_logic_probe, corrected, and parsed_as.
- Recovery prompts are less noisy.
"""
...
def print_turn(solver: Any, history: History, result: Dict[str, Any], menu: List[Dict[str, Any]], lang: str) -> None:
    ...
    ans = unique_answer(solver, history)
...
def logical_metrics(replay: ReplayRows) -> Tuple[Optional[str], Optional[int], Optional[int], int]:
    logical_answer = None
    logical_round = None
    verified_round = None
    for row in replay:
        if row.get("remaining_after") == 1 and logical_round is None:
            logical_round = int(row["round"])
            # The final answer is not always the row's guess unless the row is 4b0c.
            # The caller will fill logical_answer from current candidates when possible.
        if row.get("feedback") == "4b0c":
            verified_round = int(row["round"])
    extra = 0
    if logical_round is not None and verified_round is not None:
        extra = max(0, verified_round - logical_round)
    return logical_answer, logical_round, verified_round, extra
...
def save_replay(history: History, replay: ReplayRows, solver: Any, jackpot: bool) -> Path:
    ...
    for i in range(1, len(history)+1):
        prefix = history[:i]
        ans = unique_answer(solver, prefix)
        if ans:
            logical_answer = ans
            logical_round = i
            break
    verified_round = len(history) if solved else None
    extra = max(0, verified_round - logical_round) if verified_round and logical_round else 0
    data = {
        "project": "QuestLine",
        ...
        "extra_moves_after_logical_solve": extra,
        "history": replay,
    }
...
def solved_flow(history: History, replay: ReplayRows, solver: Any, lang: str, jackpot: bool) -> bool:
    ...
    if ask_yes_no(tr(lang, "save_prompt"), lang, default_yes=False, solved_context=True):
        print(tr(lang, "saved", path=save_replay(history, replay, solver, jackpot)))
    else:
...
def game_loop(lang: str, solver: Any, debug: bool = False) -> bool:
    ...
    while True:
        ...
        try:
            ...
            continue
        logical_before = unique_answer(solver, history)
        post_logic_probe = bool(logical_before and not (guess == logical_before and fb == (4,0)))
        # Consistency check before accepting, including 4b0c.
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    parser = argparse.ArgumentParser(description="QuestLine interactive CLI v1.2.6")
