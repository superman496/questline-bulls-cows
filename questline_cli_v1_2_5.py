"""QuestLine CLI v1.2.5

Standalone interactive CLI wrapper for the existing `questline.py` solver.

v1.2.5 is a polish patch over v1.2.4:
- Keeps the strict 2/3/6 digit input grammar.
- Hides internal debug errors by default; use `--debug` to show them.
- Error-recovery mode is less noisy.
- If a feedback correction is accepted, replay `parsed_as`, `feedback`,
  `remaining_after`, and `corrected` are synchronized.
- If a correction still leaves no candidates, it is rejected and not saved.
- Replay rows include `corrected`.

Input grammar
-------------
Commands are recognized first. For all non-command input, any non-digit
character is treated as a separator. Only these numeric structures are valid:

- 2 digits: feedback for #1, e.g. `40`, `4:0`, `4x0`, `(4,0)`.
- 3 digits: menu index + feedback, e.g. `411`, `4:11`, `4x1y1`.
- 6 digits: manual guess + feedback, e.g. `932840`, `9328:40`, `9328x4y0`.

Feedback must satisfy: 0 <= bull <= 4, 0 <= cow <= 4, bull + cow <= 4.
...
def append_replay(history: History, replay: ReplayRows, guess: str, fb: Feedback, source: str, mode: str, rank: Optional[int], solver: Any, parsed_as: str, corrected: bool = False) -> None:
    replay.append({"round": len(history), "guess": guess, "feedback": fb_text(fb), "source": source, "input_mode": mode, "matched_menu_rank": rank, "parsed_as": parsed_as, "corrected": corrected, "remaining_after": candidate_count(solver, history)})
...
def try_replace_last(raw: str, history: History, replay: ReplayRows, solver: Any, lang: str) -> Optional[bool]:
    ...
    if not history_has_candidates(solver, history):
        ...
        print(tr(lang, "correction_prompt"))
        return None
    if replay:
        row = replay[-1]
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    parser = argparse.ArgumentParser(description="QuestLine interactive CLI v1.2.5")
    parser.add_argument("--lang", choices=["zh", "en"], default=None)
    parser.add_argument("--debug", action="store_true", help="Show internal exception details in recovery mode.")
