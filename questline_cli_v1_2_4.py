"""QuestLine CLI v1.2.4 - Input Grammar

A standalone interactive CLI wrapper for the existing questline.py solver.

v1.2.4 focuses on input grammar stability:
- Commands are recognized first.
- For non-command input, any non-digit character is treated as a separator.
- Only 2 / 3 / 6 digit structures are valid:
  - 2 digits: feedback for #1, e.g. 40, 4:0, 4x0
  - 3 digits: menu index + feedback, e.g. 411, 4:11, 4x1y1
  - 6 digits: manual guess + feedback, e.g. 932840, 9328:40, 9328x4y0
- Feedback is validated before it can enter history: b/c in 0..4 and b+c <= 4.
- Manual guesses must be 4 distinct digits, leading zero allowed.
- Ambiguous grouped six-digit inputs like 123,40,1 are rejected even if
  the total number of digits is six.
- Successful input is echoed back as a standardized parsed result.

This file imports questline.py and does not modify the solver core.
"""
...
TEXT = {
    "zh": {
        ...
        "correction_rejected": "修正后仍然没有候选答案，未保存该修正。请继续输入新的反馈，或 undo 撤回上一手。",
    ...
    "en": {
        ...
        "correction_rejected": "The correction still leaves no candidates, so it was not saved. Enter another feedback or use undo.",
...
def parse_3_menu(groups: List[str], menu_len: int, lang: str) -> Tuple[int, Feedback]:
    # Allowed structures: 411 / 4,11 / 4,1,1
...
def parse_6_manual(groups: List[str], lang: str) -> Tuple[str, Feedback]:
    # Allowed structures: 932840 / 9328,40 / 9328,4,0
...
def parse_numeric_input(raw: str, menu_len: int, lang: str) -> Tuple[str, Any]:
    """Return ('feedback', fb), ('menu', choice, fb), or ('manual', guess, fb)."""
    ...
    if len(digits) == 4:
        # Usually a user entered a valid-looking guess but forgot feedback.
        try:
            guess = validate_guess_digits(digits, lang)
            raise ValueError(tr(lang, "guess_missing", guess=guess))
        except ValueError as exc:
            # repeated digits / invalid code should surface as such, not as missing feedback.
            if str(exc) == tr(lang, "digits_repeat") or str(exc) == tr(lang, "bad_guess"):
                raise
            raise
    if len(digits) == 6:
...
def append_replay(history: History, replay: ReplayRows, guess: str, fb: Feedback, source: str, mode: str, rank: Optional[int], solver: Any, parsed_as: str) -> None:
    replay.append({"round": len(history), "guess": guess, "feedback": fb_text(fb), "source": source, "input_mode": mode, "matched_menu_rank": rank, "parsed_as": parsed_as, "remaining_after": candidate_count(solver, history)})


def save_replay(history: History, replay: ReplayRows, jackpot: bool) -> Path:
    path = Path(f"questline_replay_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    data = {"project": "QuestLine", "saved_at": datetime.now().isoformat(), "solved": bool(history and history[-1][1] == (4,0)), "jackpot": jackpot, "final_answer": history[-1][0] if history and history[-1][1] == (4,0) else None, "rounds": len(history), "history": replay}
...
def solved_flow(history: History, replay: ReplayRows, lang: str, jackpot: bool) -> bool:
    print()
    if jackpot:
    ...
    if ask_yes_no(tr(lang, "save_prompt"), lang, default_yes=False, solved_context=True):
        print(tr(lang, "saved", path=save_replay(history, replay, jackpot)))
    else:
        print(tr(lang, "not_saved"))
    return ask_yes_no(tr(lang, "new_prompt"), lang, default_yes=True, solved_context=True)


def check_history_nonempty(solver: Any, history: History) -> bool:
    if not history:
        return True
    if len(history) == 1:
        return FIRST_BRANCH_COUNTS.get(history[0][1], 0) > 0
    try:
        return len(solver.filter_candidates(history)) > 0
    except Exception:
        return False
...
def try_replace_last(raw: str, history: History, replay: ReplayRows, solver: Any, lang: str) -> Optional[bool]:
    ...
    if replay:
        replay[-1]["feedback"] = fb_text(fb)
        replay[-1]["remaining_after"] = candidate_count(solver, history)
    if not check_history_nonempty(solver, history):
    ...
    if fb == (4,0):
        return solved_flow(history, replay, lang, jackpot=len(history)==1)
    return None
...
def game_loop(lang: str, solver: Any) -> bool:
    ...
    while True:
        ...
        if result is None:
            ...
            print(tr(lang, "inconsistent_hint")); continue
        menu = build_menu(solver, result, history)
        ...
        print(pline)
        append_replay(history, replay, guess, fb, source, mode, rank, solver, pline)
        if fb == (4,0):
            return solved_flow(history, replay, lang, jackpot=len(history)==1)
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    parser = argparse.ArgumentParser(description="QuestLine interactive CLI v1.2.4")
