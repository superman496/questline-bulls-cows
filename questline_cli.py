...
TEXT = {
    "zh": {
        ...
        "feedback": "反馈示例：40、4:0、4x0、4b0c、1,2、(1,2)",
    ...
    "en": {
        ...
        "audit_book_title": "Complete audit case record",
...
def print_help(lang: str) -> None:
    ...
    if lang == "zh":
        ...
        print("  --mode assist      -> Assist: you act, the engine explains")
        print("  --mode simulation  -> Simulation: you provide the answer, the engine acts")
        print("  --mode adventure   -> Adventure: the system hides the answer, you investigate")
...
def recommendation_reason(item: Dict[str, Any], lang: str = "zh") -> str:
    ...
    return questline.QuestLineSolver.public_reason(reason, lang)
...
def print_adventure_guidance(session: questline.GameSession, solver: Any, lang: str) -> None:
    ...
    task_label = questline.QuestLineSolver.public_task(task, lang)
    print(f"当前答案范围：{len(candidates)} 个可能答案" if lang == "zh" else f"Current answer range: {len(candidates)} possible answers")
    print(f"当前调查任务：{task_label}" if lang == "zh" else f"Investigation task: {task_label}")
    ...
    if recommendations:
        ...
        for index, item in enumerate(recommendations, 1):
            ...
            reason = questline.QuestLineSolver.public_reason(reason, lang)
            if lang == "zh":
...
def print_adventure_situation(session: questline.GameSession, lang: str) -> None:
    ...
    task_label = questline.QuestLineSolver.public_task(investigation.get("task"), lang)
    if lang == "zh":
        print(f"局势判断：当前答案范围还有 {state['candidate_count']} 个可能答案；当前任务是{task_label}。")
        ...
        print(f"Situation: {state['candidate_count']} possible answers remain; task: {task_label}.")
...
def strategy_state(result: Dict[str, Any], lang: str) -> str:
    ...
    task = investigation.get("task")
    if task:
        return questline.QuestLineSolver.public_task(task, lang)
    return tr(lang, "normal")


def build_menu(solver: Any, result: Dict[str, Any], history: History) -> List[Dict[str, Any]]:
    """Present the engine's own six-slot recommendations without rebuilding them.

    questline.py::choose() already computes QuestLine/AVG/MM/Conspiracy with a
    shared action/is_candidate schema and its own duplicate policy (slots are
    kept even when two of them land on the same guess). The CLI must not
    recompute AVG/MM/Conspiracy from scratch or drop slots that repeat a guess,
    or it silently falls back to a different, older recommendation policy.
    """
    ...
    return [dict(rec) for rec in result.get("recommendations", [])[:6]]
...
def print_menu(menu: List[Dict[str, Any]], lang: str, round_number: int) -> None:
    ...
    for i, item in enumerate(menu, 1):
        ...
        action_label = questline.QuestLineSolver.public_action_label(action_type, lang)
...
def print_report(solver: Any, history: History, lang: str, state: CliState, reveal_logic: bool = True, include_candidates: bool = True) -> None:
    ...
    investigation = result.get("investigation") or {}
    if investigation:
        task_text = questline.QuestLineSolver.public_task(investigation.get("task"), lang)
